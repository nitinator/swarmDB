...
connection::handle_post(const std::vector<std::string>& path)
{
    ...
    {
        LOG(debug) << "create: " << path[KEY_PATH_IDX] << " : " << post_data.str().substr(0,60);

        request.mutable_header()->set_db_uuid(path[UUID_PATH_IDX]);
        request.mutable_create()->set_key(path[KEY_PATH_IDX]);
        request.mutable_create()->set_value(post_data.str());

        // todo: temp until we remove all json api or we store the request instead of the json message...
        msg["msg"] = boost::beast::detail::base64_encode(request.SerializeAsString());

        this->crud->handle_create(msg, request, response);
        format_http_response(this->request.target(), response, this->response);
    ...
    {
        LOG(debug) << "update: " << path[KEY_PATH_IDX] << " : " << post_data.str().substr(0,60);

        request.mutable_header()->set_db_uuid(path[UUID_PATH_IDX]);
        request.mutable_update()->set_key(path[KEY_PATH_IDX]);
        request.mutable_update()->set_value(post_data.str());

        // todo: temp until we remove all json api or we store the request instead of the json message...
        msg["msg"] = boost::beast::detail::base64_encode(request.SerializeAsString());

        this->crud->handle_update(msg, request, response);
        format_http_response(this->request.target(), response, this->response);
    ...
    {
        LOG(debug) << "delete: " << path[KEY_PATH_IDX] << " : " << post_data.str().substr(0,60);

        request.mutable_header()->set_db_uuid(path[UUID_PATH_IDX]);
        request.mutable_delete_()->set_key(path[KEY_PATH_IDX]);

        // todo: temp until we remove all json api or we store the request instead of the json message...
        msg["msg"] = boost::beast::detail::base64_encode(request.SerializeAsString());

        this->crud->handle_delete(msg, request, response);
        format_http_response(this->request.target(), response, this->response);
